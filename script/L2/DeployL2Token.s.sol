...
contract DeployL2Token is Script {
    ...
    function run() external returns (address deployedL2TokenAddress) {
        ...
        require(l1TokenAddress != address(0), "L1_TOKEN_ADDRESS environment variable must be set.");
        console.log("Using L1 Token Address (L1_TOKEN_ADDRESS): %s", l1TokenAddress);

        // L2 Standard Bridge address for Ozean - https://docs.ozean.finance/developer-documentation/contract-addresses
        address l2StandardBridgeAddress = vm.envAddress("L2_STANDARD_BRIDGE_OZEAN");
        ...
        require(l2StandardBridgeAddress != address(0), "L2 Standard Bridge address for Ozean must be set and valid.");
        console.log("L2 Standard Bridge: %s", l2StandardBridgeAddress);

        vm.startBroadcast(deployerPrivateKey);

        L2Token l2Token = new L2Token(l2StandardBridgeAddress, l1TokenAddress, TOKEN_NAME_L2, TOKEN_SYMBOL_L2);
